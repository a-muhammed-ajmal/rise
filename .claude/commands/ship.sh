...
# -------------------------------------------------
# /ship Workflow Script
# - Runs lint, test:coverage, build (stops on first failure)
# - Syncs SPEC.md / CLAUDE.md / README.md metrics from the source of truth
#   (test count, coverage, AI-tool counts, DB tables, migration count)
# - Commits and pushes to main
# - Always prints a final report (via an EXIT trap), pass or fail
# Invoked by the /ship command (.claude/commands/ship.md).
# -------------------------------------------------
...
if ! npm run lint; then
  ...
  if npx eslint --fix --ext .js,.jsx,.ts,.tsx .; then
    ...
    if npm run lint; then
      ...
      echo "  - Fix them manually or adjust eslint config, then re-run bash .claude/commands/ship.sh"
      exit 1
    fi
  else
    ...
    echo "  - Then re-run bash .claude/commands/ship.sh"
    exit 1
  fi
else
  LINT_STATUS="PASS"
...
if npm run build; then
  ...
  echo "    - Re-run bash .claude/commands/ship.sh"
...
# -------------------------------------------------
# 4. Update SPEC.md — anchor first, else the Metrics table row
# -------------------------------------------------
...
# -------------------------------------------------
# 5. Update CLAUDE.md — anchor first, else the "Current: N tests, X%" line
# -------------------------------------------------
...
# -------------------------------------------------
# 6. Update README.md — metrics row + structural rows derived from source
# -------------------------------------------------
...
if git -c user.email="$USER_EMAIL" commit -m "$COMMIT_MSG"; then
  ...
  echo "  - Then re-run bash .claude/commands/ship.sh (or just push if commit succeeded externally)."
  exit 1
fi
...
if git push "$REPO_URL" "$BRANCH"; then
  ...
  echo "  - Then re-run bash .claude/commands/ship.sh (or just continue if push succeeds externally)."
  exit 1
fi
