...
version: 2.1.0
description: Single source of truth for RISE OS frontend. Deep-navy brand with a golden-yellow accent, the three-leaf mark, Inter-only typography, mandatory graph-paper texture, strict token system. Apply to every UI component and page.
---
...
| Token                | Hex            | Role |
|----------------------|----------------|------|
| `--brand`            | `#0C2443`      | Accents, borders, icon fills, focus rings — NOT filled-surface text |
| `--brand-action`     | `#0C2443`      | Filled button/FAB surfaces — white text AA (5.2:1) |
| `--brand-hover`      | `#16365C`      | Hover states |
| `--brand-text`       | `#0C2443`      | Orange text on light (AA 4.8:1) |
| `--brand-tint`       | `#EEF2F8`      | Badges, chips, icon containers |
...
`#0C2443` is only 2.93:1 (fails AA). Orange text/icons on light backgrounds use
...
/* Dark */
.graph-bg-dark {
  background-image: linear-gradient(rgba(12, 36, 67,0.07) 1px, transparent 1px),
