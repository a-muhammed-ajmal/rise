...
const ListContactsInput = z.object({
  type: z.enum(['lead', 'prospect', 'client', 'network', 'personal', 'all']).optional(),
  limit: z.number().int().positive().max(100).optional(),
})
...
const UpdateContactInput = z.object({
  ...
  type: z.enum(['lead', 'prospect', 'client', 'network', 'personal']).optional(),
...
const ListReviewsInput = z.object({
  type: z.enum(['daily', 'weekly', 'monthly', 'quarterly', 'yearly', 'all']).optional(),
...
export async function executeTool(toolName: string, input: Record<string, unknown>): Promise<ToolResult> {
  ...
  switch (toolName) {
    ...
    case 'list_projects': {
      ...
      return { success: true, message: `Found ${data?.length ?? 0} projects`, data }
    }
    ...
    case 'create_project': {
      ...
      return { success: true, message: `Created project: "${p.data.name}"`, data }
    }
    ...
    case 'delete_project': {
      ...
      const { data: existing } = await supabase.from('projects')
        .select('id, name').eq('id', p.data.project_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Project not found' }
      const { error } = await supabase.from('projects')
        .delete().eq('id', p.data.project_id).eq('user_id', user.id)
    ...
    case 'list_goals': {
      ...
      let query = supabase.from('goals')
        .select('id, title, description, category, target_date, progress, status')
        .eq('user_id', user.id)
      if (statusFilter !== 'all') query = query.eq('status', statusFilter)
      const { data, error } = await query.order('created_at', { ascending: false }).limit(50)
      if (error) return dbErr('list_goals', error)
      return { success: true, message: `Found ${data?.length ?? 0} goals`, data }
    }
    ...
    case 'update_goal': {
      ...
      if (!p.success) return badInput()
      const { id, ...updates } = p.data
      const { error, data } = await supabase.from('goals')
        .update(updates).eq('id', id).eq('user_id', user.id)
        .select('title').single()
    ...
    case 'complete_goal': {
      ...
      const { error, data } = await supabase.from('goals')
        ...
        .eq('id', p.data.id).eq('user_id', user.id)
        .select('title').single()
    ...
    case 'delete_goal': {
      ...
      const { data: existing } = await supabase.from('goals')
        .select('id, title').eq('id', p.data.goal_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Goal not found' }
      const { error } = await supabase.from('goals')
        .delete().eq('id', p.data.goal_id).eq('user_id', user.id)
    ...
    case 'create_milestone': {
      ...
      if (!p.success) return badInput()
      const { error, data } = await supabase.from('milestones').insert({
        user_id: user.id,
      ...
      return { success: true, message: `Created milestone: "${p.data.title}"`, data }
    }
    ...
    case 'list_milestones': {
      ...
      const { data, error } = await supabase.from('milestones')
        ...
        .eq('goal_id', p.data.goal_id).eq('user_id', user.id)
        .order('due_date', { ascending: true, nullsFirst: false })
      if (error) return dbErr('list_milestones', error)
      return { success: true, message: `Found ${data?.length ?? 0} milestones`, data }
    }
    ...
    case 'update_milestone': {
      ...
      if (!p.success) return badInput()
      const { id, ...updates } = p.data
      const { error, data } = await supabase.from('milestones')
        .update(updates).eq('id', id).eq('user_id', user.id)
        .select('title').single()
    ...
    case 'complete_milestone': {
      ...
      return { success: true, message: `Completed milestone: "${data?.title}"` }
    }
    ...
    case 'delete_milestone': {
      ...
      const { data: existing } = await supabase.from('milestones')
        .select('id, title').eq('id', p.data.milestone_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Milestone not found' }
      const { error } = await supabase.from('milestones')
        .delete().eq('id', p.data.milestone_id).eq('user_id', user.id)
      if (error) return dbErr('delete_milestone', error)
      return { success: true, message: `Deleted milestone: "${existing.title}"` }
    }
    ...
    case 'list_habits': {
      const [{ data: habits }, { data: todayLogs }] = await Promise.all([
        supabase.from('habits').select('id, name, icon, color, frequency, active')
          .eq('user_id', user.id).eq('active', true).order('name'),
        supabase.from('habit_logs').select('habit_id')
          .eq('user_id', user.id).eq('logged_date', today).eq('completed', true),
      ])
      const loggedIds = new Set((todayLogs ?? []).map((l: { habit_id: string }) => l.habit_id))
      const result = (habits ?? []).map((h: { id: string; name: string; icon: string; color: string; frequency: string; active: boolean }) => ({
        ...h, done_today: loggedIds.has(h.id),
      }))
      return { success: true, message: `Found ${result.length} active habits`, data: result }
    }
    ...
    case 'create_habit': {
      ...
      return { success: true, message: `Created habit: "${p.data.name}"`, data }
    }
    ...
    case 'update_habit': {
      ...
      if (!p.success) return badInput()
      const { id, ...updates } = p.data
      const { error, data } = await supabase.from('habits')
        .update(updates).eq('id', id).eq('user_id', user.id)
        .select('name').single()
    ...
    case 'delete_habit': {
      ...
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('habits')
        .select('id, name').eq('id', p.data.habit_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Habit not found' }
      const { error } = await supabase.from('habits')
        .delete().eq('id', p.data.habit_id).eq('user_id', user.id)
    ...
    case 'delete_habit_log': {
      ...
      return { success: true, message: `Removed habit log for ${p.data.logged_date}` }
    }
    ...
    case 'list_transactions': {
      ...
      const { data, error } = await query.order('date', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_transactions', error)
      return { success: true, message: `Found ${data?.length ?? 0} transactions`, data }
    }
    ...
    case 'update_transaction': {
      ...
      const { data: existing } = await supabase.from('transactions')
        .select('id').eq('id', transaction_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Transaction not found' }
      const { error } = await supabase.from('transactions')
        .update(updates).eq('id', transaction_id).eq('user_id', user.id)
    ...
    case 'delete_transaction': {
      ...
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('transactions')
        .select('id, category, amount').eq('id', p.data.transaction_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Transaction not found' }
      const { error } = await supabase.from('transactions')
        .delete().eq('id', p.data.transaction_id).eq('user_id', user.id)
    ...
    case 'list_budgets': {
      const { data, error } = await supabase.from('budgets')
        ...
        .eq('user_id', user.id).order('period_end', { ascending: false })
      if (error) return dbErr('list_budgets', error)
      return { success: true, message: `Found ${data?.length ?? 0} budgets`, data }
    }
    ...
    case 'create_budget': {
      ...
      return { success: true, message: `Created budget: ${p.data.category} AED ${p.data.amount}`, data }
    }
    ...
    case 'delete_budget': {
      ...
      const { data: existing } = await supabase.from('budgets')
        .select('id, category').eq('id', p.data.budget_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Budget not found' }
      const { error } = await supabase.from('budgets')
        .delete().eq('id', p.data.budget_id).eq('user_id', user.id)
    ...
    case 'list_debts': {
      ...
      return { success: true, message: `Found ${data?.length ?? 0} debt records`, data }
    }
    ...
    case 'create_debt': {
      ...
      const { error, data } = await supabase.from('debts').insert({
        user_id: user.id,
      ...
      return { success: true, message: `Recorded AED ${p.data.amount} ${direction} ${p.data.creditor}`, data }
    }
    ...
    case 'update_debt': {
      ...
      const { data: existing } = await supabase.from('debts')
        .select('id, creditor').eq('id', debt_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Debt not found' }
      const payload = { ...updates, ...(mark_paid ? { paid_at: new Date().toISOString() } : {}) }
      const { error } = await supabase.from('debts')
        .update(payload).eq('id', debt_id).eq('user_id', user.id)
      if (error) return dbErr('update_debt', error)
      return { success: true, message: mark_paid ? `Marked debt to ${existing.creditor} as paid.` : 'Debt updated.' }
    }
    ...
    case 'delete_debt': {
      ...
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('debts')
        .select('id, creditor').eq('id', p.data.debt_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Debt not found' }
      const { error } = await supabase.from('debts')
        .delete().eq('id', p.data.debt_id).eq('user_id', user.id)
      if (error) return dbErr('delete_debt', error)
      return { success: true, message: `Deleted debt record for ${existing.creditor}` }
    }
    ...
    case 'list_contacts': {
      ...
      return { success: true, message: `Found ${data?.length ?? 0} contacts`, data }
    }
    ...
    case 'update_contact': {
      ...
      if (!p.success) return badInput()
      const { id, ...updates } = p.data
      const { error, data } = await supabase.from('contacts')
        .update(updates).eq('id', id).eq('user_id', user.id)
        .select('name').single()
    ...
    case 'delete_contact': {
      ...
      const { data: existing } = await supabase.from('contacts')
        .select('id, name').eq('id', p.data.contact_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Contact not found' }
      const { error } = await supabase.from('contacts')
        .delete().eq('id', p.data.contact_id).eq('user_id', user.id)
    ...
    case 'create_interaction': {
      ...
      if (!p.success) return badInput()
      const { error, data } = await supabase.from('interactions').insert({
        user_id: user.id,
      ...
      return { success: true, message: `Logged ${p.data.type} interaction`, data }
    }
    ...
    case 'list_interactions': {
      ...
      const { data, error } = await supabase.from('interactions')
        ...
        .eq('contact_id', p.data.contact_id).eq('user_id', user.id)
        .order('date', { ascending: false }).limit(20)
      if (error) return dbErr('list_interactions', error)
      return { success: true, message: `Found ${data?.length ?? 0} interactions`, data }
    }
    ...
    case 'delete_interaction': {
      ...
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('interactions')
        .select('id').eq('id', p.data.interaction_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Interaction not found' }
      const { error } = await supabase.from('interactions')
        .delete().eq('id', p.data.interaction_id).eq('user_id', user.id)
    ...
    case 'list_notes': {
      ...
      const { data, error } = await query.order('created_at', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_notes', error)
      return { success: true, message: `Found ${data?.length ?? 0} notes`, data }
    }
    ...
    case 'list_documents': {
      const p = z.object({ limit: z.number().int().positive().max(100).optional() }).safeParse(input)
      const limitVal = (p.success && p.data.limit) ? p.data.limit : 20
      const { data, error } = await supabase.from('documents')
        .select('id, name, file_type, file_size, tags, notes, created_at')
        .eq('user_id', user.id).order('created_at', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_documents', error)
      return { success: true, message: `Found ${data?.length ?? 0} documents`, data }
    }
    ...
    case 'create_document': {
      ...
      return { success: true, message: `Saved document: "${p.data.name}"`, data }
    }
    ...
    case 'delete_document': {
      ...
      const { data: existing } = await supabase.from('documents')
        .select('id, name').eq('id', p.data.document_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Document not found' }
      const { error } = await supabase.from('documents')
        .delete().eq('id', p.data.document_id).eq('user_id', user.id)
    ...
    case 'list_links': {
      ...
      return { success: true, message: `Found ${data?.length ?? 0} links`, data }
    }

    case 'create_link': {
      const p = CreateLinkInput.safeParse(input)
      if (!p.success) return badInput()
      const { error, data } = await supabase.from('links').insert({
        user_id: user.id,
        url: p.data.url,
      ...
      return { success: true, message: `Saved link: "${p.data.title ?? p.data.url}"`, data }
    }
    ...
    case 'delete_link': {
      ...
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('links')
        .select('id').eq('id', p.data.id).eq('user_id', user.id).maybeSingle()
    ...
    case 'list_journal_entries': {
      const p = z.object({ limit: z.number().int().positive().max(50).optional() }).safeParse(input)
      const limitVal = (p.success && p.data.limit) ? p.data.limit : 10
      const { data, error } = await supabase.from('journal_entries')
        .select('id, date, mood, energy, tags, created_at')
        .eq('user_id', user.id).order('date', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_journal_entries', error)
      return { success: true, message: `Found ${data?.length ?? 0} journal entries`, data }
    }

    case 'create_journal_entry': {
      const p = CreateJournalEntryInput.safeParse(input)
      if (!p.success) return badInput()
      const { error, data } = await supabase.from('journal_entries').upsert({
        user_id: user.id,
        date: p.data.date,
        content: p.data.content,
      ...
      return { success: true, message: `Journal entry saved for ${p.data.date}`, data }
    }
    ...
    case 'delete_journal_entry': {
      ...
      const { data: existing } = await supabase.from('journal_entries')
        .select('id, date').eq('id', p.data.entry_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Journal entry not found' }
      const { error } = await supabase.from('journal_entries')
        .delete().eq('id', p.data.entry_id).eq('user_id', user.id)
      if (error) return dbErr('delete_journal_entry', error)
      return { success: true, message: `Deleted journal entry for ${existing.date}` }
    }
    ...
    case 'list_reviews': {
      ...
      let query = supabase.from('reviews')
        ...
        .eq('user_id', user.id)
      if (typeFilter && typeFilter !== 'all') query = query.eq('type', typeFilter)
      const { data, error } = await query.order('period_end', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_reviews', error)
      return { success: true, message: `Found ${data?.length ?? 0} reviews`, data }
    }
    ...
    case 'create_review': {
      ...
      return { success: true, message: `Created ${p.data.type} review for ${p.data.period_start}–${p.data.period_end}`, data }
    }
    ...
    case 'update_review': {
      ...
      const { error } = await supabase.from('reviews')
        .update({ ...rest, ...(content !== undefined ? { content: { text: content } } : {}) })
    ...
    case 'delete_review': {
      ...
      const { data: existing } = await supabase.from('reviews')
        .select('id, type, period_start').eq('id', p.data.review_id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Review not found' }
      const { error } = await supabase.from('reviews')
        .delete().eq('id', p.data.review_id).eq('user_id', user.id)
      if (error) return dbErr('delete_review', error)
      return { success: true, message: `Deleted ${existing.type} review for ${existing.period_start}` }
    }
    ...
    case 'list_focus_sessions': {
      ...
      const { data, error } = await query.order('started_at', { ascending: false }).limit(limitVal)
      if (error) return dbErr('list_focus_sessions', error)
      return { success: true, message: `Found ${data?.length ?? 0} focus sessions`, data }
    }
    ...
    case 'create_focus_session': {
      ...
      const { error, data } = await supabase.from('focus_sessions').insert({
        user_id: user.id,
      ...
      return { success: true, message: `Recorded ${p.data.duration_minutes}-minute focus session`, data }
    }
    ...
    case 'delete_focus_session': {
      const p = ByIdInput.safeParse(input)
      if (!p.success) return badInput()
      const { data: existing } = await supabase.from('focus_sessions')
        .select('id').eq('id', p.data.id).eq('user_id', user.id).maybeSingle()
      if (!existing) return { success: false, message: 'Focus session not found' }
      const { error } = await supabase.from('focus_sessions')
        .delete().eq('id', p.data.id).eq('user_id', user.id)
