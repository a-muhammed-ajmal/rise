...
export const AUTO_TOOLS: FunctionDeclaration[] = [
  ...
  // ─── TASKS ───────────────────────────────────────────────────────────────────
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        priority: { type: Type.STRING, enum: ['P1', 'P2', 'P3', 'P4'], description: 'Task priority — P1 (highest) to P4 (lowest); defaults to P3' },
        due_date: { type: Type.STRING, description: 'Due date in YYYY-MM-DD format' },
        due_time: { type: Type.STRING, description: '24-hour time of day for the due date, format HH:MM e.g. "09:30" or "15:00"' },
        status: { type: Type.STRING, enum: ['todo', 'in_progress', 'blocked', 'on_hold', 'done'], description: 'Task status; defaults to todo' },
        description: { type: Type.STRING, description: 'Optional task description' },
        estimated_time: { type: Type.NUMBER, description: 'Estimated duration in minutes' },
        recurrence: { type: Type.STRING, description: 'Recurrence rule (RRULE-lite). FREQ is required: FREQ=DAILY|WEEKLY|MONTHLY|YEARLY. Optional parts: INTERVAL=<n> (default 1), BYDAY=MO,TU,... (WEEKLY only), UNTIL=YYYYMMDD, COUNT=<n>. e.g. "FREQ=WEEKLY;BYDAY=MO,WE,FR" or "FREQ=DAILY;INTERVAL=2"' },
        reminder: { type: Type.STRING, description: 'ISO 8601 datetime for a reminder notification' },
        area: { type: Type.STRING, enum: ['personal', 'professional', 'financial', 'wellness', 'relationship', 'vision', 'legal', 'default'], description: 'Life area this task belongs to; defaults to default' },
        project_id: { type: Type.STRING, description: 'Project UUID to assign this task to. Call list_projects first to resolve the id.' },
        subtasks: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { title: { type: Type.STRING }, done: { type: Type.BOOLEAN } }, required: ['title'] }, description: 'Checklist items for this task' },
      },
      required: ['title'],
    },
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        filter: { type: Type.STRING, enum: ['all', 'today'], description: "Filter tasks by view — 'today' or 'all' (default)" },
      },
      required: [],
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        priority: { type: Type.STRING, enum: ['P1', 'P2', 'P3', 'P4'], description: 'P1 (highest) to P4 (lowest)' },
        status: { type: Type.STRING, enum: ['todo', 'in_progress', 'blocked', 'on_hold', 'done'] },
        due_date: { type: Type.STRING, description: 'Due date in YYYY-MM-DD format, or null to clear' },
        due_time: { type: Type.STRING, description: '24-hour time of day for the due date, format HH:MM e.g. "09:30", or null to clear' },
        description: { type: Type.STRING, description: 'Task description' },
        estimated_time: { type: Type.NUMBER, description: 'Estimated duration in minutes, or null to clear' },
        recurrence: { type: Type.STRING, description: 'Recurrence rule (RRULE-lite), e.g. "FREQ=WEEKLY;BYDAY=MO,WE,FR", or null to clear. FREQ is required: DAILY|WEEKLY|MONTHLY|YEARLY.' },
        reminder: { type: Type.STRING, description: 'ISO 8601 datetime for a reminder, or null to clear' },
        area: { type: Type.STRING, enum: ['personal', 'professional', 'financial', 'wellness', 'relationship', 'vision', 'legal', 'default'], description: 'Life area this task belongs to' },
        project_id: { type: Type.STRING, description: 'Project UUID to assign, or null to unassign' },
        is_focus: { type: Type.BOOLEAN, description: "Mark/unmark as today's focus. Only tasks due today or with no due date are eligible; capped at 3 focused tasks per day." },
        labels: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Label list (replaces existing labels)' },
        subtasks: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { id: { type: Type.STRING }, title: { type: Type.STRING }, done: { type: Type.BOOLEAN } }, required: ['title'] }, description: 'Checklist items (replaces existing subtasks)' },
      },
      required: ['id'],
    },
  },
  ...
  {
    name: 'duplicate_task',
    description: "Duplicate a task, copying its title, description, priority, due date/time, project, area, labels and estimated time. Resets recurrence, reminder, focus, and subtasks on the copy — mirrors the app's 'Duplicate Task' action.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        task_id: { type: Type.STRING, description: 'UUID of the task to duplicate' },
      },
      required: ['task_id'],
    },
  },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: Type.STRING, enum: ['personal', 'professional', 'financial', 'wellness', 'relationship', 'vision', 'legal', 'default'], description: 'Life area category; defaults to default' },
        goal_id: { type: Type.STRING, description: 'Parent goal UUID to link this project to' },
      },
      required: ['name'],
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: Type.STRING, enum: ['personal', 'professional', 'financial', 'wellness', 'relationship', 'vision', 'legal', 'default'], description: 'Life area category' },
        goal_id: { type: Type.STRING, description: 'Parent goal UUID, or null to unlink' },
      },
      required: ['id'],
    },
  },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        goal_id: { type: Type.STRING, description: 'Reassign to a different goal UUID' },
      },
      required: ['id'],
    },
  },
  ...
  },
  {
    ...
    description: 'Un-complete a milestone, clearing its completion date. Requires the milestone id.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING, description: 'Milestone UUID' },
  ...
  {
    name: 'log_habit',
    description: 'Log a habit as done (or explicitly missed) for a given date',
    parameters: {
      type: Type.OBJECT,
      properties: {
        habit_name: { type: Type.STRING, description: 'Name of the habit to log (partial match OK)' },
        logged_date: { type: Type.STRING, description: 'Date to log in YYYY-MM-DD format; defaults to today. Supports backdating.' },
        completed: { type: Type.BOOLEAN, description: 'true to mark done, false to log an explicit miss; defaults to true' },
        note: { type: Type.STRING, description: 'Optional note for this log entry' },
      },
      required: ['habit_name'],
    },
  },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        color: { type: Type.STRING, description: 'Hex color e.g. #6366f1' },
        icon: { type: Type.STRING, description: 'Emoji icon for the habit; defaults to ⭐' },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        icon: { type: Type.STRING, description: 'Emoji icon for the habit' },
  ...
  },
  {
    ...
    description: 'Edit the note, date, or completed status of an existing habit log entry. Requires the log id — call list_habit_logs first.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING, description: 'Habit log UUID' },
        logged_date: { type: Type.STRING, description: 'YYYY-MM-DD' },
  ...
  },
  {
    ...
    description: "Get full habit log history (not just today's status), optionally filtered by habit and date range",
    parameters: {
      type: Type.OBJECT,
      properties: {
        habit_id: { type: Type.STRING, description: 'Filter to one habit' },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        tags: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Tags for this transaction' },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        role: { type: Type.STRING, description: "Contact's role or title" },
        type: { type: Type.STRING, enum: ['lead', 'prospect', 'client', 'network', 'personal'] },
        stage: { type: Type.STRING, enum: ['new', 'qualified', 'proposal', 'negotiation', 'won', 'lost'], description: 'Pipeline stage; defaults to new' },
  ...
  },
  {
    ...
    description: 'List interaction history — for one contact, or across all contacts if contact_id is omitted',
    parameters: {
      type: Type.OBJECT,
      properties: {
        contact_id: { type: Type.STRING, description: 'Contact UUID — omit to list interactions across all contacts' },
        limit: { type: Type.NUMBER, description: 'Max results, default 20' },
        offset: { type: Type.NUMBER, description: 'Skip this many results — use the offset reported when more results are available' },
      },
      required: [],
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        contact_id: { type: Type.STRING, description: 'Reassign to a different contact UUID' },
      },
      required: ['id'],
    },
  },
  ...
  // ─── NOTES ────────────────────────────────────────────────────────────────────
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        linked_to_type: { type: Type.STRING, enum: ['task', 'goal', 'contact'], description: 'Link this note to another record' },
        linked_to_id: { type: Type.STRING, description: 'UUID of the linked record; must be provided together with linked_to_type' },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        tags: { type: Type.ARRAY, items: { type: Type.STRING } },
        linked_to_type: { type: Type.STRING, enum: ['task', 'goal', 'contact'], description: 'Link this note to another record, or null to unlink' },
        linked_to_id: { type: Type.STRING, description: 'UUID of the linked record; must be provided together with linked_to_type' },
      },
      required: ['id'],
    },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        started_at: { type: Type.STRING, description: 'ISO 8601 datetime' },
...
export const APPROVAL_TOOLS: FunctionDeclaration[] = [
  ...
  {
    name: 'bulk_update_task_priority',
    description: 'Change the priority of multiple tasks at once (REQUIRES USER APPROVAL)',
    parameters: {
      type: Type.OBJECT,
      properties: {
        task_ids: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Array of task UUIDs' },
        priority: { type: Type.STRING, enum: ['P1', 'P2', 'P3', 'P4'] },
        summary: { type: Type.STRING, description: 'Human-readable summary of what will change, for confirmation display' },
      },
      required: ['task_ids', 'priority', 'summary'],
    },
  },
  ...
  },
  {
    ...
    description: "Move money between two of the user's own wallets (REQUIRES USER APPROVAL). Call list_payment_methods first to resolve ids.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        from_payment_method_id: { type: Type.STRING, description: 'Source wallet UUID' },
        to_payment_method_id: { type: Type.STRING, description: 'Destination wallet UUID' },
        amount: { type: Type.NUMBER, description: 'Amount in AED, must be positive' },
        date: { type: Type.STRING, description: 'YYYY-MM-DD, defaults to today' },
        description: { type: Type.STRING, description: 'Optional note' },
        summary: { type: Type.STRING, description: 'Human-readable summary for confirmation display e.g. "Transfer AED 500 from Cash to ADCB"' },
      },
      required: ['from_payment_method_id', 'to_payment_method_id', 'amount', 'summary'],
    },
  },
  {
    name: 'create_adjustment',
    description: 'Manually correct a wallet balance up or down, with a required reason (REQUIRES USER APPROVAL). Call list_payment_methods first to resolve the id.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        payment_method_id: { type: Type.STRING, description: 'Wallet UUID' },
        amount: { type: Type.NUMBER, description: 'Signed delta in AED — positive increases the balance, negative decreases it' },
        reason: { type: Type.STRING, description: 'Required reason for the adjustment e.g. "Bank fee"' },
        date: { type: Type.STRING, description: 'YYYY-MM-DD, defaults to today' },
        summary: { type: Type.STRING, description: 'Human-readable summary for confirmation display' },
      },
      required: ['payment_method_id', 'amount', 'reason', 'summary'],
    },
  },
