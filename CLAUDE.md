...
- **Testing Standard:** Maintain ≥ 85% Vitest line coverage strictly inside `lib/**` paths (excluding `lib/types/`). Current: 609 tests, 95.04% — target met. Remaining gap is entirely `use-tasks.ts` (43%, pre-existing, not part of the last recovery pass).
