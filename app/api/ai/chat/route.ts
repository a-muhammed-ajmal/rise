...
async function buildContext(_userId: string): Promise<string> {
  const supabase = await createClient()
  const today = format(new Date(), 'yyyy-MM-dd')
  const dow = new Date().getDay()

  const [{ data: tasks }, { data: habits }, { data: goals }] = await Promise.all([
    supabase.from('tasks').select('title, priority, due_date, status').neq('status','done').or(`due_date.eq.${today},status.eq.inbox`).limit(10),
    supabase.from('habits').select('id, name').eq('active', true).contains('target_days', [dow]),
    supabase.from('goals').select('title, progress').eq('status','active').limit(5),
  ])

  const lines: string[] = []
  if (tasks?.length) lines.push(`Today's tasks (${tasks.length}): ${tasks.map((t) => `"${t.title}" [${t.priority}]`).join(', ')}`)
  if (habits?.length) lines.push(`Habits due today: ${habits.map((h) => h.name).join(', ')}`)
  if (goals?.length) lines.push(`Active goals: ${goals.map((g) => `"${g.title}" (${g.progress}%)`).join(', ')}`)
